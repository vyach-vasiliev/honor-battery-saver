...
[![Языки](https://img.shields.io/badge/интерфейс-6_языков-79E6B2?style=flat-square&labelColor=182033)](#язык-и-тема)
...
> [!IMPORTANT]
> Приложение использует недокументированный OEM WMI-интерфейс HONOR, поэтому перед постоянным использованием его необходимо проверить на целевом ноутбуке.

> [!WARNING]
> Honor Battery Saver — независимый неофициальный проект, не связанный с HONOR и не одобренный компанией. Совместимость с оборудованием не гарантируется. Программа предоставляется «как есть», без гарантий; используйте её на свой риск. Перед установкой прочитайте полный [отказ от ответственности](./DISCLAIMER.ru.md).
...
- Английский, русский, немецкий, португальский, корейский, упрощённый китайский или системный язык Windows без перезапуска приложения.
...
1. Вручную установите **Microsoft .NET 10 Desktop Runtime x64** с [официальной страницы Microsoft](https://dotnet.microsoft.com/ru-ru/download/dotnet/10.0/runtime). Обычного .NET Runtime недостаточно.
2. Запустите `HonorBatterySaverSetup.exe` и подтвердите единственный запрос прав администратора.
3. Установщик разместит приложение в `%ProgramFiles%\Honor Battery Saver`, настроит и запустит службу Windows с отложенным автоматическим стартом, добавит tray в автозапуск текущего пользователя и сможет запустить его с исходным неповышенным токеном пользователя.

Установщик поддерживает только Windows 11 x64 сборки 22000 или новее. Если Desktop Runtime 10 x64 отсутствует, он предложит открыть официальную страницу загрузки и завершится, ничего не устанавливая.

Удаляйте приложение через **Параметры → Приложения → Установленные приложения**. Деинсталлятор остановит и удалит службу, запись автозапуска текущего пользователя, файлы программы и служебные логи. `%LocalAppData%\HonorBatterySaver\settings.json` намеренно сохраняется, чтобы правила Wi-Fi пережили обновление или повторную установку; удалите этот каталог вручную, только если хотите стереть и настройки.
...
Доступны **Русский**, **English**, **Deutsch**, **Português**, **한국어**, **简体中文** и **Использовать язык Windows**. Выбор без перезапуска обновляет окно, меню в трее, уведомления, диагностику и ответы службы. Для неподдерживаемых языков интерфейса Windows используется английский (`en-US`). Светлая и тёмная темы следуют за Windows.
...
8. Все шесть языков и системный язык сохраняются и без перезапуска обновляют весь интерфейс.
...
На сайте показаны настоящие окна настроек и диагностики в тёмной теме на всех шести поддерживаемых языках. Имена сетей и диагностические статусы — демоданные, а не результаты аппаратной проверки. Для повторного создания двенадцати PNG в Windows:

```powershell
dotnet run --project tools/HonorBatterySaver.Screenshots -c Release
```

Рендерер сохраняет файлы в `site/assets/screenshots` (либо в каталог, переданный после `--`). Он использует настройки и обработчики с демоданными в памяти: не запускает рабочее приложение или службу, не запрашивает Wi-Fi, не сохраняет настройки и не отправляет команды оборудованию.
...
Полное описание локальной обработки данных находится в [политике приватности](./PRIVACY.md). Сообщайте об уязвимостях приватно в соответствии с [политикой безопасности](./SECURITY.md).

Недокументированный OEM-интерфейс может дать неожиданный результат на непроверенном устройстве или после обновления прошивки, драйвера, Windows либо PC Manager. Перед автоматическим изменением профилей прочитайте [отказ от ответственности](./DISCLAIMER.ru.md), подготовьте резервные копии и способ восстановления и проверьте аппаратную диагностику. При первом запуске приложение потребует явно подтвердить понимание этого предупреждения.

## Лицензия и подпись кода

Honor Battery Saver распространяется как открытое ПО по [лицензии MIT](./LICENSE).

Первый предварительный релиз намеренно публикуется без подписи, пока проект подаёт заявку в open-source-программу SignPath Foundation. Перед запуском установщика проверьте контрольную сумму SHA-256, приложенную к релизу GitHub. Происхождение сборки, роли участников и планируемый состав подписываемых файлов описаны в [политике подписи кода](./CODE_SIGNING_POLICY.md).
